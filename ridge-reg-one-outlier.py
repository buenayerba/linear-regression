...
def ridge_regression(X, y, reg_const):
	A = np.dot(np.transpose(X), X) + reg_const * np.eye(14)
...
def errors_for_different_lambdas(X_train, y_train, X_test, y_test, name):
	...
	for reg_const in lambdas:
		...
		percent_nonzeros.append(1.0 * np.count_nonzero(w != 0) / w.shape[0])
...
def find_valid_error(X_train, y_train):
	...
	for reg_const in lambdas:
		...
		for i in range(10):
			...
			percent_nonzeros_for_lambda += 1.0 * np.count_nonzero(w != 0) / w.shape[0]
			
		percent_nonzeros.append(percent_nonzeros_for_lambda / 10)
		square_error_ave = sum(square_errors_for_lambda) / 10
...
def main():
	...
	i_alt = 100
	trn_data[i_alt] = trn_data[i_alt] * 1000000
	trn_labels[i_alt] = trn_labels[i_alt] * 1000	 

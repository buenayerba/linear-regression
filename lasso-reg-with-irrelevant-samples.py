...
def lasso_regression(X, y, reg_const):
	...
	while True:
		...
		for j in range(w.shape[0]):
			...
			arg2 = reg_const / (X[:, j]**2).sum()
			 
			w[j] = soft_treshold(arg1, arg2)
 		iteration+=1
#   	print np.sum(np.abs(w - w0))
		if (np.sum(np.abs(w - w0)) < 10**-3) or (iteration >= 30):
			print"=====HELLO======", reg_const
			return w
...
def errors_for_different_lambdas(X_train, y_train, X_test, y_test, name):
	...
	for reg_const in lambdas:
		...
		percent_nonzeros.append(1.0 * np.count_nonzero(w[15:] != 0.0) / w.shape[0])		
...
# 			print square_error
			...
			percent_nonzeros_for_lambda += 1.0 * np.count_nonzero(w[15:] != 0.0) / w.shape[0]
